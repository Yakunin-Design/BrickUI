...
export default function DateInput(props: props) {
  ...
  useEffect(() => {
    ...
    const current_month =
      user_locale == "ru-RU"
        ...
        : today.toLocaleString(user_locale, { month: "long" });
    set_selected_month(current_month);
  }, []);

  useEffect(() => {
    set_day(props.value.getDate().toString());
    set_selected_month(
      props.value.toLocaleString("default", { month: "long" }),
    );
    set_year(props.value.getFullYear().toString());
  }, [props.value]);
  ...
  function handle_change(
    ...
    let new_date: Date | undefined = new Date(
      Number(updated_date.year),
      month_index,
      Number(updated_date.day),
    );
    if (
      updated_date.year.length < 4 ||
      !updated_date.day ||
      !Number(updated_date.day) ||
      Number(updated_date.day) <= 0 ||
      updated_date.day !== new_date.getDate().toString() ||
      month_index > 11
    )
      new_date = undefined;
